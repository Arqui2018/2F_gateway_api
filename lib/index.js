...
const url$3 = process.env.USERS_URL || 'user_ms';
const port$3 = process.env.USERS_PORT || '3000';
const entryPoint$3 = process.env.USERS_ENTRY || 'sessions';

const URL$3 = `http://${url$3}:${port$3}/${entryPoint$3}`;

const resolvers$3 = {
  Query: {
    sessionByToken: (_, { token }) => generalRequest(`${URL$3}/${token}`, 'GET'),
  },
  Mutation: {
    createSession: (_, { user }) => generalRequest(`${URL$3}`, 'POST', user),
    removeSession: (_, { token }) => generalRequest(`${URL$3}/${token}`, 'DELETE'),
  },
};
